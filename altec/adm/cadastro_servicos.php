...
if ($_POST) {
	...
	if ($_POST ['acao']=='alterar'){
		...
		if ($imagem=="")
			$sql = "update servicos set  titulo='".$_POST["titulo"]."',descricao='".$_POST["descricao"]."' where (codigo=".$_POST["codigo"].");";
		else
			$sql = "update servicos set imagem='".$imagem."', titulo='".$_POST["titulo"]."',descricao='".$_POST["descricao"]."' where (codigo=".$_POST["codigo"].");";
		//echo $sql;
		mysql_query($sql, $link);
	}
	else if( $_POST['acao']=='inserir'){
		$sql = "insert into servicos (imagem, titulo, descricao) values ('".$imagem."','".$_POST["titulo"]."','".$_POST["descricao"]."');";
		//echo $sql;
		mysql_query($sql, $link);
	}
	else if ($_POST['acao']=='excluir'){
		//delete_file($folder . $imagem_antiga);
		$sql = 'delete FROM servicos where codigo=0'.$_POST["codigo"];
		//echo $sql;
		mysql_query($sql, $link);
	}
