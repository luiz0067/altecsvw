...
if (($_GET["codigo"]!=null)){
	$sql	= "SELECT codigo,imagem,titulo,descricao FROM servicos_prestados where (codigo=0".$_GET["codigo"].")";
...
<?php
if ($_POST) {
	$folder = "..\\upload\\servicos_prestados\\";
	if (
		(
	...
	if ($_POST ['acao']=='alterar'){
		...
		if ($imagem=="")
			$sql = "update servicos_prestados set  titulo='".$_POST["titulo"]."',descricao='".$_POST["descricao"]."' where (codigo=".$_POST["codigo"].");";
		else
			$sql = "update servicos_prestados set imagem='".$imagem."', titulo='".$_POST["titulo"]."',descricao='".$_POST["descricao"]."' where (codigo=".$_POST["codigo"].");";
		//echo $sql;
		mysql_query($sql, $link);
	}
	else if( $_POST['acao']=='inserir'){
		$sql = "insert into servicos_prestados (imagem, titulo, descricao) values ('".$imagem."','".$_POST["titulo"]."','".$_POST["descricao"]."');";
		//echo $sql;
		mysql_query($sql, $link);
	}
	else if ($_POST['acao']=='excluir'){
		//delete_file($folder . $imagem_antiga);
		$sql = 'delete FROM servicos_prestados where codigo=0'.$_POST["codigo"];
		//echo $sql;
		mysql_query($sql, $link);
	}
...
?>
	...
	<form  method="post" enctype="multipart/form-data" name="form1" id="form1">
		<table >
			...
			<tr>
				<td>descricao:</td>
				<td><textarea name="descricao" ><?php echo $_GET["descricao"];?></textarea></td>
			</tr>
			<tr>
				<td colspan="2">
		...
		<?php
			...
			$sql    = "SELECT * FROM servicos_prestados s order by titulo asc;	";
			$result = mysql_query($sql, $link);
			$i=0;
			if ($result!=null) 
		...
		?>
				...
				<tr>
					...
					<td>
						...
						<img width="100px" height="100PX" src="../upload/servicos_prestados/g_<?php echo $row['imagem'];?>">
						<?php }
						else{
						?>
						&nbsp;
