		...
		<table border="1">
			...
			<?php
				...
				if( $_POST['buscar']=='buscar por codigo'){
					$sql    = "select codigo,nome FROM categoria where codigo=".$_POST["codigo"]."  order by codigo asc";
					//echo $sql;
					mysql_query($sql, $link);
				}
				else if( $_POST['buscar']=='buscar por nome'){
					$sql    = "select codigo,nome FROM categoria where nome like '%".$_POST["nome"]."%'  order by nome asc";
					//echo $sql;
					mysql_query($sql, $link);
				}
				else 
