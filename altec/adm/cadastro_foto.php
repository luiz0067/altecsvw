...
$result=null;
if (($_GET["codigo"]!=null)){
	$sql	= "SELECT codigo,imagem,nome,produto FROM fotos where (codigo=0".$_GET["codigo"].")";
...
if ($_POST) {
	...
	if ($_POST ['acao']=='alterar'){
		...
		if ($imagem=="")
			$sql = "update fotos set  nome='".$_POST["nome"]."',produto='".$_POST["produto"]."' where (codigo=".$_POST["codigo"].");";
		else
			$sql = "update fotos set imagem='".$imagem."', nome='".$_POST["nome"]."',produto='".$_POST["produto"]."' where (codigo=".$_POST["codigo"].");";
		//echo $sql;
		mysql_query($sql, $link);
	}
	else if( $_POST['acao']=='inserir'){
		$sql = "insert into fotos (imagem, nome, produto) values ('".$imagem."','".$_POST["nome"]."','".$_POST["produto"]."');";
		//echo $sql;
		mysql_query($sql, $link);
	}
	else if ($_POST['acao']=='excluir'){
		//delete_file($folder . $imagem_antiga);
		$sql = 'delete FROM fotos where codigo='.$_POST["codigo"];
		//echo $sql;
		mysql_query($sql, $link);
	}
...
?>
	...
	<form action="?produto=<?php echo $_GET["produto"]?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
		<table >
			...
			<input name="produto" type="hidden" value="<?php echo $_GET["produto"];?>">
			<tr>
				<td>nome:</td>
		...
		<?php
			...
			$sql    = "SELECT f.codigo,f.imagem,f.nome,p.nome as produto,p.codigo as codigo_produto,p.Descricao,link FROM fotos f right join produtos p on (p.codigo=f.produto) where (p.codigo=0".$_GET["produto"].");";
			$result = mysql_query($sql, $link);
			if (!$result) {
		...
		?>
				...
				<tr>
					<td><a href="?produto=<?php echo $_GET["produto"]?>&codigo=<?php echo $row['codigo'];?>"><?php echo $row['codigo'];?><a/></td>
					<td>
						<?php if($row['imagem']!=""){?>
