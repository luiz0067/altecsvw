...
$result=null;
if (($_GET["codigo"]!=null)){
	$sql	= "SELECT codigo,imagem,nome,codigo_produto FROM foto where (codigo=0".$_GET["codigo"].")";
...
if ($_POST) {
			...
			else
			{
				...
				echo "<a href=\"..\\upload\\imagens\\".$imagem."\">".$imagem."</a><br>";
					if ($_POST ['acao']=='alterar'){
						...
						$sql = "update foto set imagem='".$imagem."', nome='".$_POST["nome"]."',codigo_produto='".$_POST["codigo_produto"]."' where (codigo=".$_POST["codigo"].");";
						echo $sql;
						mysql_query($sql, $link);
					}
					else if( $_POST['acao']=='inserir'){
						$sql = "insert into foto (imagem, nome, codigo_produto) values ('".$imagem."','".$_POST["nome"]."','".$_POST["codigo_produto"]."');";
						//echo $sql;
						mysql_query($sql, $link);
					}
			}
	...
	if ($_POST['acao']=='excluir'){
		//delete_file($folder . $imagem_antiga);
		$sql = 'delete FROM foto where codigo='.$_POST["codigo"];
		//echo $sql;
		mysql_query($sql, $link);
	}
...
?>
		...
		<form action="?codigo_produto=<?php echo $_GET["codigo_produto"]?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
			codigo:<input name="codigo" type="text" value="<?php if ($result!=null) echo $row["codigo"]?>"><br>
			imagem:<input name="imagem" type="file" ><br>
			codigo_produto:<input name="codigo_produto" type="text" value="<?php echo $_GET["codigo_produto"];?>"><br>
		...
		<?php
			...
			$sql    = 'SELECT codigo,imagem,nome,link FROM foto;';
			$result = mysql_query($sql, $link);
			if (!$result) {
		...
		?>
				<tr>
					...
					<td>
						<a href="..\upload\imagens\<?php echo $row['imagem'];?>"><?php echo $row['imagem'];?><a/><br>
						<img width="50px" height="50PX" src="..\upload\imagens\<?php echo $row['imagem'];?>">
					</td>
					<td><?php echo $row['nome'];?>&nbsp </td>
					<td><a href="<?php echo $row['codigo_produto'];?>"><?php echo $row['codigo_produto'];?><a/></td>
				</tr>
			<?php
				}
				mysql_free_result($result);
